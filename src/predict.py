...
if __name__ == '__main__':
    ...
    logger.info(f'Making prediction on sample...')
    result = predict(sample)
    
    print("\n" + "="*60)
    print("PREDICTION RESULT")
    print("="*60)
    print(f"Prediction: {result['prediction']} ({'Heart Failure' if result['prediction'] == 1 else 'No Heart Failure'})")
    print(f"Probability: {result['probability']:.4f}")
    if true_label is not None:
        print(f"Actual Label: {true_label} ({'Heart Failure' if true_label == 1 else 'No Heart Failure'})")
    print("="*60 + "\n")
